...
import com.holonplatform.datastore.jdbc.internal.expressions.ExtractSQLFunction;

/**
 * Default {@link QueryFunction} resolver.
 *
 * @since 5.1.0
 */
@SuppressWarnings("rawtypes")
@Priority(Integer.MAX_VALUE)
public enum DefaultQueryFunctionResolver implements ExpressionResolver<QueryFunction, SQLFunction> {
	...
	public Optional<SQLFunction> resolve(QueryFunction expression, ResolutionContext context)
		...
		if (Year.class.isAssignableFrom(functionType))
			function = new ExtractSQLFunction("YEAR");
		if (Month.class.isAssignableFrom(functionType))
			function = new ExtractSQLFunction("MONTH");
		if (Day.class.isAssignableFrom(functionType))
			function = new ExtractSQLFunction("DAY");
		if (Hour.class.isAssignableFrom(functionType))
			function = new ExtractSQLFunction("HOUR");
