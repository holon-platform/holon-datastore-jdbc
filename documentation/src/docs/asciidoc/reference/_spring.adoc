...
*2. An additional set of properties*, provided by the link:{apidir}/com/holonplatform/jdbc/spring/JdbcDatastoreConfigProperties.html[JdbcDatastoreConfigProperties^] property set, which can be used as an alternative for the `@EnableJdbcDatastore` annotation attributes described in the previous sections.
