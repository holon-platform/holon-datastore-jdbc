...
* Directly working with `JdbcDatastore` managed _connections_, represented by the standard JDBC connection interface (`java.sql.Connection`). See <<JdbcDatastoreAPI,JdbcDatastore API>>.
...
The JDBC `Datastore` implementation supports base query-level _locks_ through the core link:{coreapidir}/com/holonplatform/core/query/lock/LockQuery.html[LockQuery^] extension.
...
IMPORTANT: The query-level lock timeout configuration may not be supported by the concrete RDBMS. When the query lock timeout is not supported, if will be simply ignored and the default RDBMS lock timeout will be used.

The `LockQuery` API is a _Datastore commodity_, automatically registered in the JDBC `Datastore` implementation. So a `LockQuery` type implementation can be otained as follows:

[source, java]
----
include::{examplesdir}/com/holonplatform/datastore/jdbc/examples/ExampleJdbcDatastoreLock.java[tag=lockcommodity,indent=0]
----
...
[source, java]
----
include::{examplesdir}/com/holonplatform/datastore/jdbc/examples/ExampleJdbcDatastoreLock.java[tag=execution,indent=0]
----
