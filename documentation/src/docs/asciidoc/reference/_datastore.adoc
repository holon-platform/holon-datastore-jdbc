...
<1> Execute an operation using a JDBC Datastore managed connection and return a `String` type result
...
==== JDBC Expression resolvers automatic registration
...
Furthermore, it makes available some API methods to invoke some JDBC Datastore operations, such as the `withConnection(ConnectionOperation<R> operation)` method to execute an operation using a Datastore managed JDBC connection or method to inspect and create other Datastore _commodities_.
...
The JDBC `Datastore` API builder provides the 
